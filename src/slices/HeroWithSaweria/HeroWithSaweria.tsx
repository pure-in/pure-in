...
const Title = ({ children }: { children: JSX.Element | JSX.Element[] }) => (
	<h2 className="text-whit title font-bold text-4xl leading-10 mb-6">{children}</h2>
);

const Paragraph = ({ children }: { children: JSX.Element | JSX.Element[] }) => (
	<p className="text-white paragraph font-light">{children}</p>
);

const HeroWithSaweria = ({ slice }: SliceComponentProps<HeroWithSaweriaType>) => {
	const { image, title, description, saweria_url } = slice.primary;
	...
	return (
		<section className="w-full hero bg-primary">
			<div className="container relative flex-col lg:flex-row flex py-10">
				...
				<div className="bottom-10 left-0 absolute w-full px-5">
					<div className="flex-bc flex-col md:flex-row bg-white bg-opacity-50 backdrop-blur-md rounded-md px-5 lg:px-10 py-4 w-full">
						...
						<h2 className=" text-2xl saweria-donation md:text-5xl font-bold">
							Rp. {convertToCurrecny(Donasi.toString())}
						</h2>
						{saweria_url && (
							<a
								className="bg-primary btn-saweria rounded-md text-white font-bold py-4 px-8 block text-center"
								href={saweria_url}
								target="_blank"
